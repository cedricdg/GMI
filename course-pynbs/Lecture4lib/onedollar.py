...
class OneDollar(object):
    ...
    def indicativeAngle(self, points):
        ...
        angle_to_rotate = np.arctan2(centroid[1] - points[0][1],
                                     centroid[0] - points[0][0])
    ...
    def rotateBy(self, points, angle):
        ...
        for point in points:
            q = np.array([0., 0.])
            q[0] = (point[0] - centroid[0]) * np.cos(angle) - (
                point[1] - centroid[1]) * np.sin(angle) + centroid[0]
            q[1] = (point[0] - centroid[0]) * np.sin(angle) + (
                point[1] - centroid[1]) * np.cos(angle) + centroid[1]
    ...
    def recognize(self, points):
        ...
        for i, template in enumerate(self.templates):
            template_points = []
            for i in range(template.shape[0]):
                template_points.append([template[i,0], template[i,1]])
            d = self.distanceAtBestAngle(points, template_points,
            ...
            if d < b:  # Get the best distance and template
                ...
                template_id = i
        score = 1 - b / (
            0.5 * np.sqrt(self.square_size**2 + self.square_size**2))
        return selected_template, template_id, score
...
def rotate2D(pts, cnt, ang=np.pi / 4):
    ...
    return np.dot(
        np.array(pts) - cnt,
        np.array([[np.cos(ang), np.sin(ang)], [-np.sin(ang),
                                               np.cos(ang)]])) + cnt
